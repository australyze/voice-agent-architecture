...
- Tell the agent to adopt roles from ai-specs/agents/ by OpenSpec phase: product-analyst (/enrich-us), ai-architect (/ff), ai-engineer (/apply), voice-ai-engineer and rag-engineer when those change types apply, evaluation-engineer (/verify), security-reviewer (/adversarial-review).
...
```yml
context: |
  Tech stack: TypeScript, Node.js, PostgreSQL, pgvector, Docker. Optional: Python, LangGraph, Langfuse, Promptfoo, DeepEval, n8n, runtime MCP. Use-case: Vapi as a voice/telephony adapter.
  Architecture: Hexagonal / Clean Architecture with provider ports. Vapi is not the application core.
  We use conventional commits
  Domain: production AI agent systems (voice, chat, RAG, tools, evaluation)
  All code, comments, documentation, and technical artifacts must be in English

  Hierarchy: docs/base-standards.md → domain docs → OpenSpec change → ai-specs/agents → skills → implementation.

  Project specs (single source of truth): All artifact creation and implementation MUST follow docs/, then the current OpenSpec change:
  - docs/base-standards.md — core principles, TDD, AI engineering, technology classification
  - docs/backend-standards.md — agents, tools, RAG, voice adapters, evaluation, observability, security
  - docs/frontend-standards.md — optional operator/HITL UI only when a frontend exists
  - docs/api-spec.yml — canonical HTTP contracts
  - docs/data-model.md — sessions, tools, retrieval, evaluation, traces
  - docs/documentation-standards.md — docs and contract maintenance
  - docs/openspec-tasks-mandatory-steps.md — gates by change type
  Adopt the relevant agent from ai-specs/agents/ for the OpenSpec phase. Use ai-specs/skills/ for procedures.

# Per-artifact rules (optional)
# Add custom rules for specific artifacts.
rules:
  # Global: apply docs when creating any artifact
  _global:
    - Before creating any artifact, read and apply docs/base-standards.md
    - /enrich-us: adopt ai-specs/agents/product-analyst.md
    - /ff and design: adopt ai-specs/agents/ai-architect.md; add voice-ai-engineer.md or rag-engineer.md when those change types apply
    - /apply: adopt ai-specs/agents/ai-engineer.md (plus voice/RAG specialists for their surfaces)
    - /verify: adopt ai-specs/agents/evaluation-engineer.md
    - /adversarial-review: adopt ai-specs/agents/security-reviewer.md and skill adversarial-review
...
Skills live in `ai-specs/skills/` and are mirrored into `.claude/skills/` and `.cursor/skills/` via relative pointers, so any copilot can discover them. Load a skill when the request matches its description (`docs/base-standards.md` § Project Skills). Skills compose with OpenSpec; they do not replace `/ff`, `/apply`, or `/verify`.

**SDD (AI):** `enrich-us` → `design-ai-system` (during `/ff`) → `implement-spec` (during `/apply`) → `verify-ai-implementation` (during `/verify`) → `adversarial-review` → `commit`.

**Surfaces:** `design-agent`, `design-tool`, `design-voice-agent`, `design-rag-pipeline`, `design-prompt`, `create-evals`, `instrument-ai-system`, `analyze-agent-failure`.

**Platform:** `using-git-worktrees`, `update-docs`, `openspec-sync-specs`, `show-spec-working`, `writing-skills`, `sync-agent-symlinks`, `code-auditing`, `explain`, `meta-prompt` (coding-agent prompts only).

See each `ai-specs/skills/<name>/SKILL.md` for when to use, when not to use, and verification criteria.
