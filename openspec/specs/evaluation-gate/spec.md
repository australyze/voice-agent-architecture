...
The system MUST expose a documented quality-gate run that executes a versioned, PII-free evaluation set covering the existing generative surfaces: the demo agent/prompt suite, the retrieval suite, and the voice conversation suite as regression. The set MUST record suite name, dataset version, and the prompt or retrieval versions under test. Cases MUST assert contracts, tool sequences, retrieval hit correctness, or labeled scores — not live-model prose.
